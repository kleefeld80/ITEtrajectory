function eigi=ITEbeyn2D(n,mux,muy,RA,RB,ior)
    eigi=[];
    NN=1;
    m=NN;
    
    f=@(k)-sqrt(ior)*besselj(n,k)*besseljp(n,k*sqrt(ior))+besseljp(n,k)*besselj(n,k*sqrt(ior));
...
function z=besseljp(n,x)
    z=n*(1./x).*besselj(n,x)-besselj(n+1,x); 
end
