function  [k_m,sigma_min]=MFS_ITE_clover(m,init,Rrel_S,n)
...
% Generation of computational points 
    ...
    phi=phi(1:end-1); 
    boundary_x=0.25*(sin(4*phi)+5).*cos(phi);
    boundary_y=0.25*(sin(4*phi)+5).*sin(phi);
    boundary=[boundary_x ; boundary_y];
    
    % Normal vectors along boundary
    normal_x=0.25*sin(4*phi).*cos(phi)+cos(4*phi).*sin(phi)+5/4*cos(phi);
    normal_y=0.25*sin(4*phi).*sin(phi)-cos(4*phi).*cos(phi)+5/4*sin(phi);
    normals=[normal_x ; normal_y]./sqrt(normal_x.^2+normal_y.^2);
    
    % Source points 
    sources=[Rrel_S*boundary_x ; Rrel_S*boundary_y];
    %sources=[Rrel_S*cos(phi) ; Rrel_S*sin(phi)];
